...
output "batch_canary_tile_shard_job_definition_arn" {
  value       = try(aws_batch_job_definition.map_tile_shard_canary[0].arn, null)
  description = "Operator-only canary tile-shard definition used for parallel validation."
}
